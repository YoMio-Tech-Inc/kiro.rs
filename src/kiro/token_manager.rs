...
/// 多凭据 Token 管理器
///
/// 支持多个凭据的管理，实现同优先级随机选择 + 优先级降级的负载均衡策略
...
impl MultiTokenManager {
        ...
        let entries: Vec<CredentialEntry> = credentials
            ...
            .map(|mut cred| {
                ...
                CredentialEntry {
                    id,
                    credentials: cred,
                    failure_count: if disabled { MAX_FAILURES_PER_CREDENTIAL } else { 0 },
                    disabled,
                    disabled_reason,
                    success_count: 0,
                    total_requests: 0,
    ...
    /// 获取当前优先级最高的可用凭据的克隆
    ///
    /// 注意：负载均衡模式下，每次请求会随机选择凭据，
    /// 此方法仅用于获取一个代表性凭据（如用于日志或调试）
    pub fn credentials(&self) -> KiroCredentials {
    ...
    pub async fn acquire_context(&self) -> anyhow::Result<CallContext> {
        self.acquire_context_excluding(&HashSet::new()).await
    }

    /// 获取 API 调用上下文（排除指定凭据）
    ///
    /// 用于重试时排除已尝试过的凭据，实现同优先级随机选择 + 优先级降级
    ///
    /// # Arguments
    /// * `excluded_ids` - 要排除的凭据 ID 集合（已尝试过的凭据）
    pub async fn acquire_context_excluding(
        ...
        let mut local_excluded = excluded_ids.clone();
        ...
        loop {
            ...
            let (id, credentials) = {
                ...
                // 使用随机选择算法选择凭据
                let selected = self.select_random_credential_inner(&entries, &local_excluded);

                if let Some((id, creds)) = selected {
                    (id, creds)
                } else {
            ...
            match self.try_ensure_token(id, &credentials).await {
                ...
                Err(e) => {
                    ...
                    // Token 刷新失败，将该凭据加入排除列表（不计入失败次数）
                    local_excluded.insert(id);
                    tried_count += 1;
                }
            }
        }
    ...
    /// 从可用凭据中随机选择一个（内部方法，需要持有 entries 锁）
    ///
    /// 选择策略：
    /// 1. 获取所有可用凭证（未禁用 + 不在排除列表中）
    /// 2. 找到最高优先级（最小 priority 值）
    /// 3. 从该优先级的凭证中随机选择一个
    fn select_random_credential_inner(
        &self,
        ...
        // 找到最高优先级
        let min_priority = available.iter().map(|e| e.credentials.priority).min().unwrap();
        ...
        let selected = candidates[fastrand::usize(..candidates.len())];
        Some((selected.id, selected.credentials.clone()))
    }

    /// 从可用凭据中随机选择一个（公开方法，用于测试）
    ///
    /// 选择策略：
    /// 1. 获取所有可用凭证（未禁用 + 不在排除列表中）
    /// 2. 找到最高优先级（最小 priority 值）
    /// 3. 从该优先级的凭证中随机选择一个
    pub fn select_random_credential(
        &self,
        excluded_ids: &HashSet<u64>,
    ) -> Option<(u64, KiroCredentials)> {
        let entries = self.entries.lock();
        self.select_random_credential_inner(&entries, excluded_ids)
    }
    ...
    fn persist_credentials(&self) -> anyhow::Result<bool> {
        ...
        // 收集所有凭据，同步运行时禁用状态到 KiroCredentials
    ...
    pub fn report_failure(&self, id: u64) -> bool {
        let (should_persist, has_available) = {
            let mut entries = self.entries.lock();
            ...
            let should_persist = if failure_count >= MAX_FAILURES_PER_CREDENTIAL {
                entry.disabled = true;
            ...
            (should_persist, has_available)
        };

        // 禁用时持久化状态
        if should_persist {
            if let Err(e) = self.persist_credentials() {
                tracing::warn!("禁用凭据后持久化失败: {}", e);
            }
        }
    ...
    ///
    /// # 返回
    /// - `Ok(Vec<u64>)` - 删除的凭据 ID 列表
    /// - `Err(_)` - 持久化失败
    pub fn delete_all_disabled(&self) -> anyhow::Result<Vec<u64>> {
        let deleted_ids: Vec<u64> = {
            let mut entries = self.entries.lock();
            let ids: Vec<u64> = entries.iter().filter(|e| e.disabled).map(|e| e.id).collect();
        ...
        if !deleted_ids.is_empty() {
            self.persist_credentials()?;
            tracing::info!("已批量删除 {} 个禁用凭据: {:?}", deleted_ids.len(), deleted_ids);
        }
...
mod tests {
    ...
    #[test]
    fn test_multi_token_manager_random_selection() {
        let config = Config::default();
        let mut cred1 = KiroCredentials::default();
        ...
        cred2.priority = 0; // 同优先级

        let manager =
            MultiTokenManager::new(config, vec![cred1, cred2], None, None, false).unwrap();

        // 多次选择，验证随机性（同优先级应该随机选择）
        let mut selected_ids = std::collections::HashSet::new();
        for _ in 0..20 {
            if let Some((id, _)) = manager.select_random_credential(&HashSet::new()) {
                selected_ids.insert(id);
            }
        }
        // 由于是随机选择，20 次应该能选到两个不同的凭据
        assert_eq!(selected_ids.len(), 2, "同优先级凭据应该被随机选择");
    }

    #[test]
    fn test_multi_token_manager_priority_degradation() {
        let config = Config::default();
        let mut cred1 = KiroCredentials::default();
        cred1.refresh_token = Some("token1".to_string());
        cred1.priority = 0; // 高优先级
        let mut cred2 = KiroCredentials::default();
        cred2.refresh_token = Some("token2".to_string());
        cred2.priority = 1; // 低优先级

        let manager =
            MultiTokenManager::new(config, vec![cred1, cred2], None, None, false).unwrap();

        // 不排除任何凭据时，应该选择高优先级的凭据
        let (id, _) = manager.select_random_credential(&HashSet::new()).unwrap();
        assert_eq!(id, 1, "应该选择高优先级凭据");

        // 排除高优先级凭据后，应该降级到低优先级
        let mut excluded = HashSet::new();
        excluded.insert(1);
        let (id, _) = manager.select_random_credential(&excluded).unwrap();
        assert_eq!(id, 2, "排除高优先级后应该降级到低优先级凭据");
    }

    #[tokio::test]
    async fn test_multi_token_manager_acquire_context_no_auto_recovery() {
        let config = Config::default();
        let mut cred1 = KiroCredentials::default();
    ...
    #[test]
    fn test_disabled_state_loaded_from_credentials() {
        // 测试从 KiroCredentials 加载禁用状态
        let config = Config::default();
        let mut cred1 = KiroCredentials::default();
        cred1.disabled = true;
        cred1.disabled_reason = Some("too_many_failures".to_string());
        ...
        // cred1 应该被禁用，cred2 应该可用
        assert_eq!(manager.total_count(), 2);
        assert_eq!(manager.available_count(), 1);

        // 验证只能选择 cred2
        let (id, _) = manager.select_random_credential(&HashSet::new()).unwrap();
        assert_eq!(id, 2, "应该只能选择未禁用的凭据");
    }

    #[test]
    fn test_disabled_reason_string_conversion() {
        // 测试 DisabledReason 字符串转换
        assert_eq!(DisabledReason::Manual.as_str(), "manual");
        assert_eq!(DisabledReason::TooManyFailures.as_str(), "too_many_failures");
        assert_eq!(DisabledReason::QuotaExceeded.as_str(), "quota_exceeded");

        assert_eq!(DisabledReason::from_str("manual"), Some(DisabledReason::Manual));
        assert_eq!(
            DisabledReason::from_str("too_many_failures"),
            Some(DisabledReason::TooManyFailures)
        );
        assert_eq!(
            DisabledReason::from_str("quota_exceeded"),
            Some(DisabledReason::QuotaExceeded)
        );
        assert_eq!(DisabledReason::from_str("unknown"), None);
    }
